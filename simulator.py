...
strip while remaining inside the *marginal* ranges — the case Isolation Forest
is supposed to be for. Hour is generated on a 24h clock (later encoded as sin/cos).
"""
...
SCENARIO_HINTS = {
    "brute_force": "T1110 (illustrative) — 1D, failed logins",
    "rate_exfil": "T1041 (illustrative) — joint, bytes/duration off-manifold",
    "low_and_slow": "T1029 (illustrative) — joint, long + low rate",
}
...
def _on_manifold(rng: np.random.Generator, n: int) -> tuple[np.ndarray, np.ndarray]:
...
def generate_sessions(
    ...
    bytes_sent, duration = _on_manifold(rng, n_normal)
    ...
    attacks = [
        ...
        _low_and_slow(rng, n_per_scenario),
    ]
...
def _brute_force(rng: np.random.Generator, n: int) -> pd.DataFrame:
    ...
    bytes_sent, duration = _on_manifold(rng, n)
...
def _low_and_slow(rng: np.random.Generator, n: int) -> pd.DataFrame:
